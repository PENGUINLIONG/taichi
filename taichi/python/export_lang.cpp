...
#undef PER_EXTENSION
  ...
  py::class_<Program>(m, "Program")
      ...
      .def("create_texture", [&](Program * program, BufferFormat fmt,
                                 const std::vector<int> &shape) -> Texture * {
            return program->create_texture(fmt, shape);
          },
          py::arg("fmt"),
...
#undef PER_TYPE
  ...
  m.def("get_primitive_data_type", [](uint32_t type) {
    switch ((PrimitiveTypeID)type) {
#define PER_TYPE(x) \
    case PrimitiveTypeID::x:\
